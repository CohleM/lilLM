...
@torch.no_grad()
def estimate_losses(model, sft_dataset, batch_size, block_size, device, eval_iters, device_type, ctx):
    out = {}
    model.eval()
...
def main(args):
    ...
    if args.init_from == "scratch":
        ...
        apply_lora(model, r=args.low_rank)
    ...
    while True:
        ...
        if num_iter % args.eval_interval == 0 and master_process:
            losses = estimate_losses(model,sft_dataset, args.batch_size, args.block_size, device, args.eval_iters, device_type, ctx)
            print(
            ...
            if losses["val"] < best_val_loss:
                ...
                #     "config": model_config,
                # }
                save_lora(model, output_path='best_model_sft_lora.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--low_rank", type=int, default=DEFAULT_LOW_RANK, help="rank value")
